...
下图展示了牛模型在相机环绕视角下的动态渲染效果。实验通过不断改变相机的方位角 `azim`，从多个角度对模型进行渲染，并将连续帧保存为 GIF 动图。（特别注意，gif旋转一圈后就会停止不动，如需观察完整旋转过程，可刷新）

![牛模型旋转渲染结果](cow_rotation.gif)
...
![不同视角渲染结果](cow_views_compare.png)
...
![不同光照条件渲染结果](cow_lights_compare.png)
